...
```bash
FORCE_CPU=true fastapi run main.py --port 5200
```
The environment variable is unnecessary if only the CPU is available.

The application will begin running at `http://localhost:5200` if the port was not specified, or at `http://localhost:PORT_NUMBER` if a different port was specified.
...
***Run the container***
   ```shell
      docker run -p 5200:5200 fastwhisperapi
   ```
### Google Colab
...
```bash
curl -X POST "http://localhost:5200/v1/transcriptions" \
